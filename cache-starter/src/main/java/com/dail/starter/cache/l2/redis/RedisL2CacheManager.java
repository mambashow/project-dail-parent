...
/**
 * description Redis二级缓存管理器
 *
 * @author Dail 2023/01/03 15:40
 */
public class RedisL2CacheManager extends RedisCacheManager implements L2CacheManager {
    ...
    public RedisL2CacheManager(RedisConnectionFactory connectionFactory) {
        super(new DefaultRedisCacheWriter(connectionFactory),
    ...
    protected RedisCache getMissingCache(String name) {
        ...
        if (expiresSeconds != null) {
            return createRedisCache(name,
                    RedisCacheConfiguration.defaultCacheConfig().disableCachingNullValues().entryTtl(Duration.ofSeconds(expiresSeconds)));
        } else {
            return createRedisCache(name,
                    RedisCacheConfiguration.defaultCacheConfig().disableCachingNullValues());
        }
